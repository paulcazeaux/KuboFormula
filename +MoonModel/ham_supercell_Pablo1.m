function [ Hmat] = ham_supercell_Pablo1( know, total_num, inplaneGAB_table_bottom,inplaneGAB_vecs_bottom, inplaneGAB_table_top,inplaneGAB_vecs_top,inplaneGAABB_table_bottom,inplaneGAABB_vecs_bottom, inplaneGAABB_table_top,inplaneGAABB_vecs_top,inplane_hops,inter_table,inter_vec,inter_hop)
    ...
    for ind=1:21
        ...
        Hmat_inplane(tmp_index_b)=Hmat_inplane(tmp_index_b)+NGAB_hoppings(ind)*exp(-1i*dot(know,delvec_b));
        Hmat_inplane(tmp_index_t)=Hmat_inplane(tmp_index_t)+NGAB_hoppings(ind)*exp(-1i*dot(know,delvec_t));
    ...
    for ind=1:18
        ...
        Hmat_inplane(tmp_index_b)=Hmat_inplane(tmp_index_b)+NGAABB_hoppings(ind)*exp(-1i*dot(know,delvec_b));
        Hmat_inplane(tmp_index_t)=Hmat_inplane(tmp_index_t)+NGAABB_hoppings(ind)*exp(-1i*dot(know,delvec_t));
    ...
    for inds=1:intsize(1)
        Hmat_interaction(tmp_index(inds))=Hmat_interaction(tmp_index(inds))+inter_hop(inds)*exp(-1i*inter_vec(inds,:)*know');
    end
