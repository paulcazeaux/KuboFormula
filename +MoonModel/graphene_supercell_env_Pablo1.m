...
inplane_ds=[ 1.42, 2.46   , 2.8406,3.7577,4.2608,4.9200,5.1209, 5.6811 ];

%inplane_hops_Pablo1=[-2.7,-0.27,0,0,0,0,0,0];

inplane_hops_Pablo1=-2.7*exp(-(inplane_ds-1.42)/0.184/sqrt(3)/1.42);

inplane_hops=inplane_hops_Pablo1;
...
num_pos_b=ind-1;
fprintf('Number for all_pos_layer1: %f\n', num_pos_b);
...
num_pos_t=ind-1;
fprintf('Number for all_pos_layer2: %f\n', num_pos_t);
...
bottom_angle_a=angle(deltav_b(1) + 1i*deltav_b(2));
top_angle_a=angle(deltav_t(1) + 1i*deltav_t(2));
    ...
    for indt=1:num_pos_t
        ...
        for indl=1:num_list
            ...
            if mindist<=inter_cutoff
                ...
                angle3=angle(minvec(1)+1i*minvec(2));
